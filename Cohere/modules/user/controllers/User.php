...
class User extends CI_Controller
{
	...
	function edit_profile()
	{
		...
		if($this->input->post('hash') && sha1($this->session->userdata('userName')) == $this->input->post('hash'))
		{
			...
			$this->form_validation->set_rules('designation', phrase('designation'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('organisation', phrase('organisation'), 'trim|required|xss_clean');
	...
	function forget()
	{
		...
		if(null != $this->input->post('hash'))
		{
			$this->form_validation->set_rules('email', phrase('email_address'), 'trim|required|valid_email');
			...
			{
				$email = $this->input->post('email');
    $findemail = $this->Actions_model->ForgotPassword($email);
    if ($findemail) {
        $this->Actions_model->sendpassword($findemail);
    } else {
        echo json_encode(array('status' => 406, 'messages' => phrase('your_email_id_doesnt_exists')));
        redirect(base_url() . 'user/forget', 'refresh');
    }
			}
		}
		else
		{
			...
			if($this->input->is_ajax_request())
			{
				...
				$this->output->set_output(
					json_encode(
						array(
							'meta'		=> $data['meta'],
							'html'		=> $this->load->view('forget', $data, true)
						)
					)
				);
			}
			...
			{
				$this->template->build('forget', $data);
			}
		}
	}
	...
	function register()
 	{
		...
		if(null != $this->input->post('hash'))
		{
			...
			$this->form_validation->set_rules('gender', phrase('gender'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('age', phrase('age'), 'trim|required|xss_clean|numeric');
			$this->form_validation->set_rules('dob', phrase('date_of_birth'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('mobile', phrase('mobile'), 'trim|required|xss_clean|numeric');
			$this->form_validation->set_rules('address', phrase('address'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('branch', phrase('branch'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('year', phrase('year'), 'trim|required|xss_clean|numeric|min_length[4]');
			$this->form_validation->set_rules('designation', phrase('designation'), 'trim|required|xss_clean');
			$this->form_validation->set_rules('organisation', phrase('organisation'), 'trim|required|xss_clean');
