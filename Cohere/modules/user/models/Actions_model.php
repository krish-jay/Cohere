    ...
    if ($query1->num_rows()>0)
{
        ...
        $newpass['password'] = sha1($passwordplain.SALT);
        $this->db->where('email', $email);
        $this->db->update('users', $newpass);
        $subject =phrase('your_one_time_password');
        $config				= array();
			...
			$message_body		= '
				<p>' . phrase('thank_you') . ', <b>' . $this->input->post('email') . '</b>. ' . phrase('your_password_has_been_updated_with_one_time_password') . '<br /><br /><b>' . phrase('here_is_your_one_time_usable_password') . ':</b></p><br />
					<table width="100%">
						<tr>
							<td valign="top">
								' . phrase('your_new_one_time_password_is_,') . '
							</td>
							<td valign="top">
								:
							</td>
					...
					<br /><br />
					<b>' . phrase('*_please_change_your_password_after_login') . '</b>
				</div>
			';
		...
		$this->email->from('no-reply@' . $_SERVER['SERVER_NAME'], $this->settings['siteTitle']);
			...
			$this->email->cc($email);
			$this->email->subject($subject . ' - ' . $this->settings['siteTitle']);
            ...
            echo json_encode(array('status' => 406, 'messages' => phrase('something_went_wrong')));
        } else {

            echo json_encode(array('status' => 200, 'messages' => phrase('your_one_time_password_is_sent_to_your_mail')));
        }
    ...
    else
    {
        ...
        echo json_encode(array('status' => 406, 'messages' => phrase('something_went_wrong_with_mail_id')));
